...
public class CommandMovieNight implements Command {
    ...
    public List<Long> getPermissions() {
        return Arrays.asList(MovieRoles.STAFF.id, MovieRoles.MOVIENIGHT.id);
    }
        ...
        if (matcherOption.find()) {
            ...
            if (option.equalsIgnoreCase("set")) {
                ...
                if (matcherSet.find()) {
                    ...
                    IMessage thingy = Util.sendBufferedMessage(guild.getChannelByID(MovieBot.ANNOUNCEMENT_CH_ID), announcement);
                    String messageID = thingy.getStringID();

                    Util.sendBufferedMessage(guild.getChannelByID(MovieBot.GENERAL_CH_ID), announcement);
                    try {
                ...
                if (matcherAnnounce.find()) {
                    ...
                    IMessage message2Delete = Util.sendBufferedMessage(guild.getChannelByID(MovieBot.ANNOUNCEMENT_CH_ID), announcement);
