...
public class CommandMovieNight implements Command {
    ...
    public void execute(MovieBot bot, IDiscordClient client, String[] args, IGuild guild, IMessage message, boolean isPrivate) {
        ...
        if (roles.contains(guild.getRoleByID(MovieRoles.ADMIN.id)) || roles.contains(guild.getRoleByID(MovieRoles.MOVIENIGHT.id))) {
            ...
            if (matcherOption.find()) {
                ...
                if (option.equalsIgnoreCase("set")) {
                    ...
                    if (matcherSet.find()) {
                        ...
                        IMessage thingy = Util.sendBufferedMessage(guild.getChannelByID(MovieBot.ANNOUNCEMENT_CHANNEL_ID), announcement);
                        String messageID = thingy.getID();

                        Util.sendBufferedMessage(guild.getChannelByID(MovieBot.GENERAL_CHANNEL_ID), announcement);
                        try {
                    ...
                    if (matcherAnnounce.find()) {
                        ...
                        IMessage message2Delete = Util.sendBufferedMessage(guild.getChannelByID(MovieBot.ANNOUNCEMENT_CHANNEL_ID), announcement);
